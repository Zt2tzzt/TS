...
2014年，Facebook 推出了 flow 来对 JavaScript 进行类型检查；
...
每一样新技术的流行，都会带来惊喜，因为他必然是解决了之前技术的某一个痛点；
...
声明对象中属性的类型时，一般使用类型推导，或者 `type` 或 `interface` 关键字定义类型或接口。
