...
一个属性不希望外界任意修改，只能读取，那么可以使用 `readonly` 属性修饰符。
...
在 TypeScript 中，抽象类里，一个方法，没有具体没有方法体，就是抽象方法。
...
// TS 中类型检测通过，因为使用鸭子类型检测，在 Java 中会报错:
...
被类实现后，需要传入接口类型的地方，可以将这个类的实例传入；这就是**面向接口开发**；
...
如果索引签名中有定义其他属性的类型, 该类型也必须是 `string` 类型属性值类型的“子类型”（更加确定的类型）。
