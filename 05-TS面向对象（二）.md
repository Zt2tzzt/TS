...
一个属性不希望外界任意修改，而是获取后直接使用，那么可以使用 `readonly` 属性修饰符。
...
使用存取器 `getter` 和 `setter` 可以对类中的属性读写进行拦截。
...
TypeScript 提供了特殊的语法，可以把类中的构造函数参数，转成一个同名同值的成员属性。 这些参数就被称为**参数属性（parameter properties）**；

通过在构造函数参数前，显示地添加修饰符 `public` `private` `protected` 或者 `readonly` 来创建参数属性，成员属性也会得到这些修饰符；
...
继承是多态使用的前提。在定义通用的调用接口时, 我们通常会让调用者传入父类，通过多态来实现更加灵活的调用方式。

父类本身可能并不需要对某些方法进行具体的实现，所以可以把这类方法，定义为抽象方法。

在 TypeScript 中，抽象类里，没有具体实现的方法（没有方法体），就是抽象方法。
...
// 鸭子类型，在 Java 中会报错: 不允许
calcArea({ getArea: function() {} })
```

# 五、TS 类型检测-鸭子类型

TS 中类型检测使用的是鸭子类型，区别于传统的面向对象，不会进行严格的检测，**只关心属性和行为**, 不关心你具体是不是对应的类型
...
const person: Person = new Dog("果汁", 5) // Dog 和 Person 中的属性和方法相同，类型检测通过。
```

# 六、TS 类的类型

TS 中：类可以创建实例对象。本身也可以作为实例的类型。也可以被当作一个有构造签名的函数。
...
function factory(ctor: new () => void) {} // 有构造签名的函数
factory(Person)
```

# 七、TS 对象类型的修饰符

**对象类型的修饰符（Property Modifiers）**：

可选属性（Optional Properties）修饰符：
- 我们可以在属性名后面加一个 `?` 标记表示这个属性是可选的；

只读属性（Readonly Properties）修饰符：
- 在 TypeScript 中，属性被标记为 `readonly`，表示该属性只能读取，不能被写入。
...
接口可以被类实现的：类也可以实现多个接口。

被类实现后，需要传入接口类型的地方，可以将这个类传入；这就是面向接口开发；
...
一个索引签名，属性类型必须是 `string` 或 `number` 中的一个。
...
- `names` 是一个数组类型，
- 可以使用 `number` 类型的索引访问 `names` 中的元素外，
- 还可以调用 `names` 上的 `forEach`、`map` 等方法，本质上是通过 `string` 类型的属性名，访问对应的函数值。

因为索引签名会进行严格字面量赋值检测。不像对象类型有“类型擦除”的效果。所以下方的索引签名检验会报错。
...
因为，本质上，在访问一个对象的属性时，`number` 类型的索引，会转成 `string` 类型。

```typescript
const arr = ['abc', 'cba', 'nba']

// 在进行属性访问时，arr[9] 会转成 arr['0']
...
如果索引签名中有定义其他属性的类型, 该类型必须是 `string` 类型属性值类型的“子类型”（更加确定的类型）。
