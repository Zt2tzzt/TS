...
在一个 TS 脚文件中，变量和类型默认会被声明在共享的**全局作用域**。

一个 TS 文件，没有任何 `import` 或者 `export` 语句，但希望它被作为模块处理，需要添加这行代码：

```typescript
export {}
```

这样，文件变为一个没有导出任何内容的模块。
...
内置类型声明是 Typescript 自带的、其中内置了 JavaScript 运行时的标准化 API 声明文件；
...
> 实际开发中，一般不使用 ts-loader 编译 ts 代码，而是使用 babel 编译 ts 代码，
>
...
- 比如 react 框架不是用 TS 编写的，源码中没有 `.d.ts` 文件，需要使用安装 TS 的类型声明：
...
实际开发中，编写业务逻辑时，用到的类型，有必要在 `.d.ts` 文件中声明，推荐与业务逻辑代码放在一起，用到时声明即可。

当项目中有全局的标识符，在业务逻辑代码中使用时。才需要在 `.d.ts` 文件中声明。
...
在声明模块的内部，可以通过 ` export` 导出对应库中的类、函数等；
...
比如在开发 vue + ts 项目时，引入 `.vue` 文件默认是不识别的，需要在 `.d.ts` 文件中，对其进行文件的声明；
...
比如在 ts 项目中，引入 `.jpg` 这类图片文件时，默认也是不支持的，需要对其进行声明；
