...
通过 `type` 可以用来声明一个对象类型：也可以通过接口 `interface` 来声明：
...
```typescript
type NewType = number & string // never 没有意义
...
TS 对于作为参数传入的函数，它的参数个数是不做校验的，两个例子：
