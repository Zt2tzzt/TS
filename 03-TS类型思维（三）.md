...
TypeScript 无法获取具体的类型信息时，需要使用类型断言（Type Assertions）。

- 比如 `document.getElementById`，TypeScript 只知道该函数会返回 `HTMLElement` ，但并不知道它具体的类型：
...
```typescript
const age: number = 18
const age2 = age as string // 会直接报错。

// TS 类型检测来说是正确的, 但是不推荐这么写
const age3 = age as any
const age4 = age3 as string
console.log(age4.split(" "))
```

类型断言，最主要的目的是，方便对变量（标识符）的使用（赋值，操作）
...
函数类型的表达式中，函数的形参必写，类型可选，不写默认为 `any`。
...
// TS 类型检测通过，对“不新鲜”的对象，检测时会进行类型擦除
