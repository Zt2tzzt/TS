...
TypeScript 将使用 `new` 操作符调用的函数当作是一个构造函数（constructors）。
...
- 有默认值的情况下, 参数的类型注解可以省略，会进行推导。
- 有默认值的参数, 可以接收一个 `undefined` 的值，会使用默认值。
...
在 TypeScript 中，如果我们编写了一个 `add` 函数，希望可以对 `string` 或者 `number` 类型的参数进行相加。
...
对某些需要取出参数中特定属性进行操作的函数，可使用对象类型指明参数的类型，扩展性更强。
...
TS 要求 class 必须声明**成员属性**，类型可选，默认是 `any`。
